...
class PPCache : Cache {
 ...
 private:
  ...
  bool checkCacheIsValid(PathId cacheFileId, const ::PPCache::Reader& root) const;
  ...
  void cacheErrors(::PPCache::Builder builder, SymbolTable& targetSymbols, const ErrorContainer* errorContainer,
  ...
  void cacheVObjects(::PPCache::Builder builder, const FileContent* fC, SymbolTable& targetSymbols,
                     const SymbolTable& sourceSymbols, PathId fileId);

  // Store macros in cache. Canonicalize strings and store in "targetSymbols".
  void cacheMacros(::PPCache::Builder builder, SymbolTable& targetSymbols, const SymbolTable& sourceSymbols,
                   const std::vector<MacroInfo*>& globalMacros);

  void cacheDefines(::PPCache::Builder builder, SymbolTable& targetSymbols, const SymbolTable& sourceSymbols);

  void cacheTimeInfos(::PPCache::Builder builder, SymbolTable& targetSymbols, const SymbolTable& sourceSymbols);

  void cacheLineTranslationInfos(::PPCache::Builder builder, SymbolTable& targetSymbols,
                                 const SymbolTable& sourceSymbols);

  void cacheIncludeFileInfos(::PPCache::Builder builder, SymbolTable& targetSymbols, const SymbolTable& sourceSymbols,
                             const std::vector<MacroInfo*>& globalMacros);

  bool restore(PathId cacheFileId, bool errorsOnly, int32_t recursionDepth);

  void restoreMacros(SymbolTable& targetSymbols, const ::capnp::List<::Macro>::Reader& sourceGlobalMacros,
  ...
  void restoreTimeInfos(SymbolTable& targetSymbols, const ::capnp::List<::TimeInfo>::Reader& sourceTimeInfos,
                        const SymbolTable& sourceSymbols);

  void restoreLineTranslationInfos(SymbolTable& targetSymbols,
                                   const ::capnp::List<::LineTranslationInfo>::Reader& sourceLineTranslationInfos,
                                   const SymbolTable& sourceSymbols);

  void restoreIncludeFileInfos(SymbolTable& targetSymbols,
