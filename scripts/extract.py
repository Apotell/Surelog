      ...
      with tarfile.open(fileobj=tarfile_strm) as archive_strm:
        ...
        while not queue.empty():
          ...
          with tarfile.open(fileobj=archive_strm.extractfile(test_filepath)) as test_strm:
            ...
            if 'tsv' in modes:
              for platform_id in _platform_ids:
                src_tsv_filepath = f'{name}/{name}{platform_id}.tsv'
                if src_tsv_filepath in test_strm.getnames():
                  dst_tsv_filepath = dst_dirpath / f'{name}{platform_id}.tsv'

                  log(f'{src_tsv_filepath} => {dst_tsv_filepath}')

                  try:
                    src_tsv_strm = test_strm.extractfile(src_tsv_filepath)

                    with dst_tsv_filepath.open('wb') as dst_tsv_strm:
                      dst_tsv_strm.write(src_tsv_strm.read())
                      dst_tsv_strm.flush()

                    # On Windows, fixup the line endings
                    if is_windows():
                      with dst_tsv_filepath.open() as istrm:
                        lines = istrm.readlines()
                      with dst_tsv_filepath.open('wt') as ostrm:
                        ostrm.writelines(lines)
                        ostrm.flush()

                  except KeyError:
                    log(f'Failed to extract \"{src_tsv_filepath}\"')
                    traceback.print_exc()
                    result += 1
...
def _main():
  ...
  parser.add_argument('modes', nargs='+', choices=['db', 'log', 'tsv'], type=str, help='Pick what to extract')
  parser.add_argument(
