...
_blacklisted_typenames = set(['ErrorNode', 'EveryRule', 'Terminal'])
...
def _get_implemented_methods(filepath: str):
    parse_method_name_regex = re.compile(
        '.+::(?P<method_name>(enter|exit|visit)\w+)\s*\(.*')
...
def _generate_XXXTreeShapeListener_h(listener: str,
                                     antlr_definition_filepath: str,
                                     cpp_input_filepath: str,
                                     output_header_filepath: str):
    content = [
        '// This file is auto-generated by generate_parser_listener.py',
        '// DO NOT EDIT', '', '/*', ' Copyright 2019 Alain Dargelas',
        ...
        ' * Author: alain', ' *', ' * Created on April 16, 2017, 8:28 PM',
        ' */', ''
    ]
    ...
    if listener == 'Parser':
        ...
        content.extend([
            ...
            '#define SURELOG_SV3_1ATREESHAPELISTENER_H', '#pragma once', '',
            '#include <Surelog/SourceCompile/SV3_1aTreeShapeHelper.h>',
            '#include <parser/SV3_1aParserBaseListener.h>', '',
            'namespace SURELOG {', '  class ParseFile;', '  class Session;',
            '',
            '  class SV3_1aTreeShapeListener : public SV3_1aParserBaseListener, public SV3_1aTreeShapeHelper {',
            '  private:', '', '  public:',
            '    SV3_1aTreeShapeListener(Session *session, ParseFile* pf, antlr4::CommonTokenStream* tokens, uint32_t lineOffset);',
        ...
        content.extend([
            ...
            '#define SURELOG_SV3_1APPTREESHAPELISTENER_H', '#pragma once', '',
            '#include <Surelog/SourceCompile/PreprocessFile.h>',
            '#include <Surelog/SourceCompile/SV3_1aPpTreeListenerHelper.h>',
            '#include <parser/SV3_1aPpParserBaseListener.h>', '',
            'namespace SURELOG {', '  class Session;', '',
            '  class SV3_1aPpTreeShapeListener : public SV3_1aPpParserBaseListener , public SV3_1aPpTreeListenerHelper {',
            '', '  public:',
            '    SV3_1aPpTreeShapeListener(Session *session, PreprocessFile* pp, antlr4::CommonTokenStream* tokens, PreprocessFile::SpecialInstructions& instructions);',
            ''
        ])

    parse_method_name_regex = re.compile(
        '\s*virtual\s+void\s+(?P<method_name>(enter|exit|visit)\w+)\s*\(.*')
    sub_regex1 = re.compile(
        '\s*virtual\s+(?P<declaration>void\s+(?P<method>(enter|exit|visit)\w+).+)\s+override\s+\{\s+\}'
    )
            ...
            if line:
                ...
                if m:
                    ...
                    if method_name in implemented_methods:
                        line = sub_regex1.sub('    \g<declaration> final;',
                                              line)
                    elif method_name.startswith('exit'):
                        method_name = method_name.replace('exit', '')
                        line = sub_regex1.sub(
                            f'\g<declaration> final {{ addVObject(ctx, VObjectType::{vot_prefix}{method_name}); }}',
                            line)
    ...
    content.extend(['  };', '} // namespace SURELOG', ''])
...
def _generate_VObjectTypes_h(pp_typenames: list, pa_typenames: list,
                             filepath: str):
    ...
    content.extend([
        ...
        '} // namespace SURELOG', '#endif // SURELOG_VOBJECTTYPES_H', ''
    ])

    _write_output(filepath, '\n'.join(content))


def _generate_VObjectTypes_cpp(pp_typenames: list, pa_typenames: list,
                               filepath: str):
    global _sl_typename_start
    global _pa_typename_start
    global _pp_typename_start
    ...
    for typename in _sl_typenames:
        content.append(
            f'    case VObjectType::sl{typename} /* = {index} */: return "sl{typename}";'
        )
        index += 1
    ...
    for typename in pp_typenames:
        content.append(
            f'    case VObjectType::pp{typename} /* = {index} */: return "pp{typename}";'
        )
        index += 1
    ...
    for typename in pa_typenames:
        content.append(
            f'    case VObjectType::pa{typename} /* = {index} */: return "pa{typename}";'
        )
        index += 1
...
def _generate_VObjectTypes_py_h(pp_typenames: list, pa_typenames: list,
                                filepath: str):
    global _sl_typename_start
    global _pa_typename_start
    global _pp_typename_start
    ...
    content.extend(['};', '', '#endif // SURELOG_VOBJECTTYPES_PY_H', ''])

    _write_output(filepath, '\n'.join(content))


def _generate_SV3_1aPpParseTreeListener_cpp(tokens: list, rules: list,
                                            template_filepath: str,
                                            output_filepath: str):
    rule_case_statements = [
        f'    case SV3_1aPpParser::Rule{rule}: addVObject(ctx, VObjectType::pp{rule}); break;'
        for rule in rules
    ]
...
def _generate_SV3_1aParseTreeListener_cpp(tokens: list, rules: list,
                                          template_filepath: str,
                                          output_filepath: str):
    rule_case_statements = [
        f'    case SV3_1aParser::Rule{rule}: nodeId = addVObject(ctx, VObjectType::pa{rule}); break;'
        for rule in rules
    ]
    visit_case_statements = [
...
def _generate_ParseTreeListener_h(pp_tokens: list, pp_rules: list,
                                  pa_tokens: list, pa_rules: list,
                                  template_filepath: str,
                                  output_filepath: str):
    ...
    for prefix, rules in [('PP', pp_rules), ('PA', pa_rules)]:
        for rule in rules:
            ...
            private_listen_declarations.append(
                f'  void listen{prefix}_{rule}(const ParseTreeNode& node);')
        private_listen_declarations.append('')

    public_visit_declarations = []
    public_visit_declarations.extend([
        f'  virtual void visitSL_{typename}(const ParseTreeNode& node) {{}}'
        for typename in _sl_typenames
    ])
    public_visit_declarations.append('')
    public_visit_declarations.extend([
        f'  virtual void visitPP_{token}(const ParseTreeNode& node) {{}}'
        for token in pp_tokens
    ])
    public_visit_declarations.append('')
    public_visit_declarations.extend([
        f'  virtual void visitPA_{token}(const ParseTreeNode& node) {{}}'
        for token in pa_tokens
    ])
...
def _generate_ParseTreeListener_cpp(pp_tokens: list, pp_rules: list,
                                    pa_tokens: list, pa_rules: list,
                                    template_filepath: str,
                                    output_filepath: str):
    ...
    listen_case_statements.extend([
        f'    case VObjectType::pp{rule}: enter(node); listenPP_{rule}(node); leave(node); break;'
        for rule in pp_rules
    ])
    listen_case_statements.append('')
    listen_case_statements.extend([
        f'    case VObjectType::pa{rule}: enter(node); listenPA_{rule}(node); leave(node); break;'
        for rule in pa_rules
    ])
    listen_case_statements.append('')
    listen_case_statements.extend([
        f'    case VObjectType::sl{typename}: visit(node); visitSL_{typename}(node); break;'
        for typename in _sl_typenames
    ])
    listen_case_statements.append('')
    listen_case_statements.extend([
        f'    case VObjectType::pp{token}: visit(node); visitPP_{token}(node); break;'
        for token in pp_tokens
    ])
    listen_case_statements.append('')
    listen_case_statements.extend([
        f'    case VObjectType::pa{token}: visit(node); visitPA_{token}(node); break;'
        for token in pa_tokens
    ])
...
def _generate_ParseTreeTraceListener_h(pp_tokens: list, pp_rules: list,
                                       pa_tokens: list, pa_rules: list,
                                       template_filepath: str,
                                       output_filepath: str):
    public_enter_leave_declarations = []
    ...
    public_visit_declarations = []
    for prefix, tokens in [('SL', _sl_typenames), ('PP', pp_tokens),
                           ('PA', pa_tokens)]:
        public_visit_declarations.extend([
            f'  void visit{prefix}_{token}(const ParseTreeNode& node) final {{ TRACE_VISIT; }}'
            for token in tokens
        ])
        public_visit_declarations.append('')
...
def _main():
    ...
    parser.add_argument('--input-dirpath',
                        dest='input_dirpath',
                        required=False,
                        type=str,
    ...
    _generate_XXXTreeShapeListener_h(
        'Parser',
        os.path.join(args.output_dirpath, 'src', 'parser',
                     'SV3_1aParserBaseListener.h'),
        os.path.join(args.input_dirpath, 'src', 'SourceCompile',
                     'SV3_1aTreeShapeListener.cpp'),
        os.path.join(args.output_dirpath, 'include', 'Surelog',
                     'SourceCompile', 'SV3_1aTreeShapeListener.h'))

    _generate_XXXTreeShapeListener_h(
        'PreProc',
        os.path.join(args.output_dirpath, 'src', 'parser',
                     'SV3_1aPpParserBaseListener.h'),
        os.path.join(args.input_dirpath, 'src', 'SourceCompile',
                     'SV3_1aPpTreeShapeListener.cpp'),
        os.path.join(args.output_dirpath, 'include', 'Surelog',
                     'SourceCompile', 'SV3_1aPpTreeShapeListener.h'))

    pp_tokens, pp_rules = _collect_typenames(
        os.path.join(args.output_dirpath, 'src', 'parser',
                     'SV3_1aPpParser.interp'))
    ...
    if pp_errors:
        print(
            'Preprocessor tokens and rules cannot share the same name. Following are errors:'
        )
    ...
    if pa_errors:
        print(
            'Parser tokens and rules cannot share the same name. Following are errors:'
        )
    ...
    _generate_VObjectTypes_h(
        ...
        os.path.join(args.output_dirpath, 'include', 'Surelog',
                     'SourceCompile', 'VObjectTypes.h'))

    _generate_VObjectTypes_cpp(
        pp_typenames, pa_typenames,
        os.path.join(args.output_dirpath, 'src', 'SourceCompile',
                     'VObjectTypes.cpp'))

    _generate_VObjectTypes_py_h(
        pp_typenames, pa_typenames,
        os.path.join(args.output_dirpath, 'include', 'Surelog', 'API',
                     'VObjectTypes_py.h'))

    _generate_ParseTreeListener_h(
        pp_tokens, pp_rules, pa_tokens, pa_rules,
        os.path.join(args.input_dirpath, 'include', 'Surelog', 'SourceCompile',
                     'ParseTreeListener.template.hpp'),
        os.path.join(args.output_dirpath, 'include', 'Surelog',
                     'SourceCompile', 'ParseTreeListener.h'))

    _generate_ParseTreeTraceListener_h(
        pp_tokens, pp_rules, pa_tokens, pa_rules,
        os.path.join(args.input_dirpath, 'include', 'Surelog', 'SourceCompile',
                     'ParseTreeTraceListener.template.hpp'),
        os.path.join(args.output_dirpath, 'include', 'Surelog',
                     'SourceCompile', 'ParseTreeTraceListener.h'))

    _generate_ParseTreeListener_cpp(
        pp_tokens, pp_rules, pa_tokens, pa_rules,
        os.path.join(args.input_dirpath, 'src', 'SourceCompile',
                     'ParseTreeListener.template.cxx'),
        os.path.join(args.output_dirpath, 'src', 'SourceCompile',
