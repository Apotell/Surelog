...
_tar_filename_pattern = 'sl-{buildno}-linux-gcc-release-regression-{index}.tar.gz'
...
def _main():
  ...
  output_filepath = input_dirpath / f"sl-{args.build_no}.txt"
  ...
  input_filepaths = [input_dirpath / filename_pattern.format(buildno=args.build_no, index=i) for i in range(8)]
  ...
  if max_workers == 0:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers*2) as executor:
      for errs, cats in executor.map(scanner, input_filepaths):
        errors.extend(errs)
        categories = _merge_categories(categories, cats)
