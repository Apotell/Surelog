...
PPCache::PPCache(Session* session, PreprocessFile* pp) : Cache(session), m_pp(pp) {}
...
void PPCache::cacheErrors(::PPCache::Builder builder, SymbolTable& targetSymbols, const ErrorContainer* errorContainer,
...
void PPCache::cacheVObjects(::PPCache::Builder builder, const FileContent* fC, SymbolTable& targetSymbols,
...
bool PPCache::checkCacheIsValid(PathId cacheFileId, const ::PPCache::Reader& root) const {
  FileSystem* const fileSystem = m_session->getFileSystem();
  ...
  if (!checkIfCacheIsValid(sourceHeader, kSchemaVersion, cacheFileId, m_pp->getFileId(LINE1))) {
    return false;
  }
  ...
  const ::capnp::List<::capnp::Text, ::capnp::Kind::BLOB>::Reader& sourceSymbols = root.getSymbols();

  // Check if the defines match
  const ::capnp::List<::capnp::Text, ::capnp::Kind::BLOB>::Reader& sourceDefines = root.getDefines();
  ...
  for (const auto& definePair : targetDefines) {
    std::string spath = StrCat(m_pp->getSymbol(definePair.first), "=", definePair.second);
  ...
  const ::capnp::List<::IncludeFileInfo, ::capnp::Kind::STRUCT>::Reader& sourceIncludeFileInfos =
      root.getIncludeFileInfos();
  PathIdSet targetIncludedFileIds;
  for (const ::IncludeFileInfo::Reader& sourceIncludeFileInfo : sourceIncludeFileInfos) {
    IncludeFileInfo::Context context = static_cast<IncludeFileInfo::Context>(sourceIncludeFileInfo.getContext());
    IncludeFileInfo::Action action = static_cast<IncludeFileInfo::Action>(sourceIncludeFileInfo.getAction());
    if ((context == IncludeFileInfo::Context::Include) && (action == IncludeFileInfo::Action::Push)) {
      std::string cachedFile = fileSystem->remap(sourceSymbols[sourceIncludeFileInfo.getSectionFileId()].cStr());
      PathId cachedFileId = fileSystem->toPathId(cachedFile, symbols);
      PathId sessionFileId = fileSystem->locate(sourceSymbols[sourceIncludeFileInfo.getSymbolId()].cStr(),
  ...
  if (!std::all_of(targetIncludedFileIds.begin(), targetIncludedFileIds.end(),
                   [this](const PathId& targetIncludedFileId) {
                     return checkCacheIsValid(getCacheFileId(targetIncludedFileId));
                   })) {
    return false;
  }
...
void PPCache::cacheMacros(::PPCache::Builder builder, SymbolTable& targetSymbols, const SymbolTable& sourceSymbols,
  ...
  for (size_t index = 0, nindex = globalMacros.size(); index < nindex; ++index) {
    const MacroInfo* const info = globalMacros[index];
    ::Macro::Builder targetMacro = targetGlobalMacros[index];

    targetMacro.setNameId((RawSymbolId)targetSymbols.registerSymbol(info->m_name));
    targetMacro.setDefType(static_cast<::MacroDefType>(info->m_defType));
    targetMacro.setFileId((RawPathId)fileSystem->copy(info->m_fileId, &targetSymbols));
    ...
    const std::vector<LineColumn>& sourceArgumentPositions = info->m_argumentPositions;
    ::capnp::List<::LineColumn, ::capnp::Kind::STRUCT>::Builder targetArgumentPositions =
    ...
    const std::vector<LineColumn>& sourceTokenPositions = info->m_tokenPositions;
    ::capnp::List<::LineColumn, ::capnp::Kind::STRUCT>::Builder targetTokenPositions =
  ...
  ::capnp::List<::uint16_t, ::capnp::Kind::PRIMITIVE>::Builder targetLocalMacros =
      builder.initLocalMacros(localMacros.size());
  ...
  for (size_t index = 0, nindex = localMacros.size(); index < nindex; ++index) {
    ...
    targetLocalMacros.set(
        index, std::distance(globalMacros.begin(), std::find(globalMacros.begin(), globalMacros.end(), info)));
  }
}

void PPCache::cacheDefines(::PPCache::Builder builder, SymbolTable& targetSymbols, const SymbolTable& sourceSymbols) {
  CommandLineParser* const clp = m_session->getCommandLineParser();

  const auto& sourceDefines = clp->getDefineList();
  ::capnp::List<::capnp::Text, ::capnp::Kind::BLOB>::Builder targetDefines = builder.initDefines(sourceDefines.size());

  size_t index = 0;
  for (const auto& entry : sourceDefines) {
    std::string define = StrCat(m_pp->getSymbol(entry.first), "=", entry.second);
    targetDefines.set(index, define.c_str());
    ++index;
  }
}

void PPCache::cacheTimeInfos(::PPCache::Builder builder, SymbolTable& targetSymbols, const SymbolTable& sourceSymbols) {
  FileSystem* const fileSystem = m_session->getFileSystem();

  const std::vector<TimeInfo>& sourceTimeInfos = m_pp->getCompilationUnit()->getTimeInfo();
  ...
  ::capnp::List<::TimeInfo, ::capnp::Kind::STRUCT>::Builder targetTimeInfos = builder.initTimeInfos(count);
...
void PPCache::cacheLineTranslationInfos(::PPCache::Builder builder, SymbolTable& targetSymbols,
                                        const SymbolTable& sourceSymbols) {
  FileSystem* const fileSystem = m_session->getFileSystem();

  const std::vector<PreprocessFile::LineTranslationInfo>& sourceLineTranslationInfos = m_pp->getLineTranslationInfo();
  ::capnp::List<::LineTranslationInfo, ::capnp::Kind::STRUCT>::Builder targetLineTranslationInfos =
      builder.initLineTranslations(sourceLineTranslationInfos.size());

  for (size_t i = 0, ni = sourceLineTranslationInfos.size(); i < ni; ++i) {
    const PreprocessFile::LineTranslationInfo& sourceLineTranslationInfo = sourceLineTranslationInfos[i];
    ::LineTranslationInfo::Builder targetLineTranslationInfo = targetLineTranslationInfos[i];
    PathId pretendFileId = fileSystem->copy(sourceLineTranslationInfo.m_pretendFileId, &targetSymbols);

    targetLineTranslationInfo.setPretendFileId((RawPathId)pretendFileId);
    targetLineTranslationInfo.setOriginalLine(sourceLineTranslationInfo.m_originalLine);
    targetLineTranslationInfo.setPretendLine(sourceLineTranslationInfo.m_pretendLine);
  }
}

void PPCache::cacheIncludeFileInfos(::PPCache::Builder builder, SymbolTable& targetSymbols,
                                    const SymbolTable& sourceSymbols, const std::vector<MacroInfo*>& globalMacros) {
  FileSystem* const fileSystem = m_session->getFileSystem();

  const std::vector<IncludeFileInfo>& sourceIncludeFileInfos = m_pp->getIncludeFileInfo();
  ::capnp::List<::IncludeFileInfo, ::capnp::Kind::STRUCT>::Builder targetIncludeFileInfos =
  ...
  for (size_t i = 0, ni = sourceIncludeFileInfos.size(); i < ni; ++i) {
    ...
    ::IncludeFileInfo::Builder targetIncludeFileInfo = targetIncludeFileInfos[i];

    SymbolId symbolId = targetSymbols.copyFrom(sourceIncludeFileInfo.m_symbolId, &sourceSymbols);
    PathId sectionFileId = fileSystem->copy(sourceIncludeFileInfo.m_sectionFileId, &targetSymbols);

    targetIncludeFileInfo.setContext(static_cast<uint16_t>(sourceIncludeFileInfo.m_context));
    targetIncludeFileInfo.setAction(static_cast<uint16_t>(sourceIncludeFileInfo.m_action));
    ...
    if (sourceIncludeFileInfo.m_macroDefinition == nullptr) {
      ...
      targetIncludeFileInfo.setDefinition(
          std::distance(globalMacros.begin(),
                        std::find(globalMacros.begin(), globalMacros.end(), sourceIncludeFileInfo.m_macroDefinition)));
    }
    ...
    targetIncludeFileInfo.setSectionColumn(sourceIncludeFileInfo.m_sectionColumn);
    ...
    targetIncludeFileInfo.setIndexOpposite(sourceIncludeFileInfo.m_indexOpposite);

    const std::vector<MacroInfo*>& sourceDefinitions = sourceIncludeFileInfo.m_macroDefinitions;
    ::capnp::List<::uint16_t, ::capnp::Kind::PRIMITIVE>::Builder targetDefinitions =
        targetIncludeFileInfo.initDefinitions(sourceDefinitions.size());
    for (size_t j = 0, nj = sourceDefinitions.size(); j < nj; ++j) {
      targetDefinitions.set(j, std::distance(globalMacros.begin(), std::find(globalMacros.begin(), globalMacros.end(),
                                                                             sourceDefinitions[j])));
    }

    const std::vector<LineColumn>& sourceTokenPositions = sourceIncludeFileInfo.m_tokenPositions;
    ::capnp::List<::LineColumn, ::capnp::Kind::STRUCT>::Builder targetTokenPositions =
        targetIncludeFileInfo.initTokenPositions(sourceTokenPositions.size());
...
void PPCache::restoreMacros(SymbolTable& targetSymbols, const ::capnp::List<::Macro>::Reader& sourceGlobalMacros,
                            const ::capnp::List<::uint16_t, ::capnp::Kind::PRIMITIVE>::Reader& sourceLocalMacros,
  ...
  for (const ::Macro::Reader& sourceMacro : sourceGlobalMacros) {
    ...
    const ::capnp::List<::capnp::Text, ::capnp::Kind::BLOB>::Reader& sourceArguments = sourceMacro.getArguments();
    const ::capnp::List<::capnp::Text, ::capnp::Kind::BLOB>::Reader& sourceTokens = sourceMacro.getTokens();
    const ::capnp::List<::LineColumn, ::capnp::Kind::STRUCT>::Reader& sourceArgumentPositions =
        sourceMacro.getArgumentPositions();
    const ::capnp::List<::LineColumn, ::capnp::Kind::STRUCT>::Reader& sourceTokenPositions =
        sourceMacro.getTokenPositions();
    ...
    for (const ::LineColumn::Reader& sourcePosition : sourceArgumentPositions) {
      argumentPositions.emplace_back(sourcePosition.getLine(), sourcePosition.getColumn());
    }
    ...
    for (const ::LineColumn::Reader& sourcePosition : sourceTokenPositions) {
      tokenPositions.emplace_back(sourcePosition.getLine(), sourcePosition.getColumn());
    }

    globalMacros.emplace_back(new MacroInfo(
        sourceSymbols.getSymbol(SymbolId(sourceMacro.getNameId(), UnknownRawPath)),
        static_cast<MacroInfo::DefType>(sourceMacro.getDefType()),
        fileSystem->toPathId(
            fileSystem->remap(sourceSymbols.getSymbol(SymbolId(sourceMacro.getFileId(), UnknownRawPath))),
            &targetSymbols),
        sourceMacro.getStartLine(), sourceMacro.getStartColumn(), sourceMacro.getEndLine(), sourceMacro.getEndColumn(),
        sourceMacro.getNameColumn(), sourceMacro.getBodyColumn(), args, argumentPositions, tokens, tokenPositions));
  }
...
void PPCache::restoreTimeInfos(SymbolTable& targetSymbols, const ::capnp::List<::TimeInfo>::Reader& sourceTimeInfos,
                               const SymbolTable& sourceSymbols) {
  FileSystem* const fileSystem = m_session->getFileSystem();
...
void PPCache::restoreLineTranslationInfos(
    SymbolTable& targetSymbols, const ::capnp::List<::LineTranslationInfo>::Reader& sourceLineTranslationInfos,
    const SymbolTable& sourceSymbols) {
  FileSystem* const fileSystem = m_session->getFileSystem();

  for (const ::LineTranslationInfo::Reader& sourceLineTranslationInfo : sourceLineTranslationInfos) {
    PathId pretendFileId = fileSystem->toPathId(fileSystem->remap(sourceSymbols.getSymbol(SymbolId(
                                                    sourceLineTranslationInfo.getPretendFileId(), UnknownRawPath))),
                                                &targetSymbols);
    PreprocessFile::LineTranslationInfo lineFileInfo(pretendFileId, sourceLineTranslationInfo.getOriginalLine(),
                                                     sourceLineTranslationInfo.getPretendLine());
    m_pp->addLineTranslationInfo(lineFileInfo);
  }
}

void PPCache::restoreIncludeFileInfos(SymbolTable& targetSymbols,
  ...
  for (const ::IncludeFileInfo::Reader& sourceIncludeFileInfo : sourceIncludeFileInfos) {
    SymbolId sectionSymbolId =
        targetSymbols.copyFrom(SymbolId(sourceIncludeFileInfo.getSymbolId(), "<unknown"), &sourceSymbols);
    PathId sectionFileId = fileSystem->toPathId(
        fileSystem->remap(sourceSymbols.getSymbol(SymbolId(sourceIncludeFileInfo.getSectionFileId(), UnknownRawPath))),
        &targetSymbols);
    int32_t macroIndex = sourceIncludeFileInfo.getDefinition();
    MacroInfo* const macroInstance = (macroIndex != -1) ? macros[macroIndex] : nullptr;
    ...
    int32_t index = m_pp->addIncludeFileInfo(
        static_cast<IncludeFileInfo::Context>(sourceIncludeFileInfo.getContext()),
        static_cast<IncludeFileInfo::Action>(sourceIncludeFileInfo.getAction()), macroInstance, sectionFileId,
        sourceIncludeFileInfo.getSectionLine(), sourceIncludeFileInfo.getSectionColumn(),
        sourceIncludeFileInfo.getSourceLine(), sourceIncludeFileInfo.getSourceColumn(), sectionSymbolId,
        sourceIncludeFileInfo.getSymbolLine(), sourceIncludeFileInfo.getSymbolColumn(),
        sourceIncludeFileInfo.getIndexOpposite());

    IncludeFileInfo& fi = m_pp->getIncludeFileInfo(index);

    ::capnp::List<::uint16_t, ::capnp::Kind::PRIMITIVE>::Reader sourceDefinitions =
        sourceIncludeFileInfo.getDefinitions();
    ...
    ::capnp::List<::LineColumn, ::capnp::Kind::STRUCT>::Reader sourceTokenPositions =
        sourceIncludeFileInfo.getTokenPositions();
...
bool PPCache::restore(PathId cacheFileId, bool errorsOnly, int32_t recursionDepth) {
  if (!cacheFileId) return false;
  ...
  do {
    ...
    restoreMacros(*targetSymbols, root.getGlobalMacros(), root.getLocalMacros(), sourceSymbols, globalMacros);
    ...
    if (recursionDepth == 0) {
      ...
      restoreLineTranslationInfos(*targetSymbols, root.getLineTranslations(), sourceSymbols);
    }

    // Restore include file info
    if (recursionDepth == 0) m_pp->clearIncludeFileInfo();
    restoreIncludeFileInfos(*targetSymbols, root.getIncludeFileInfos(), sourceSymbols, globalMacros);
    ...
    if (fC == nullptr) {
      fC = new FileContent(m_session, m_pp->getFileId(0), m_pp->getLibrary(), nullptr, BadPathId);
      m_pp->setFileContent(fC);
      m_pp->getCompileSourceFile()->getCompiler()->getDesign()->addPPFileContent(m_pp->getFileId(0), fC);
    }
...
bool PPCache::save() {
  ...
  if (fC && (fC->getVObjects().size() > Cache::Capacity)) {
    ...
    errors->addError(ErrorDefinition::CMD_CACHE_CAPACITY_EXCEEDED, Location(BadSymbolId));
    return false;
  }
  ...
  for (const IncludeFileInfo& ifi : m_pp->getIncludeFileInfo()) {
    globalMacros.insert(globalMacros.end(), ifi.m_macroDefinitions.cbegin(), ifi.m_macroDefinitions.cend());
  }
  ...
  cacheErrors(builder, targetSymbols, errorContainer, *sourceSymbols, subjectFileId);
