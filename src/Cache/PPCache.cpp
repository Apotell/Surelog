...
void PPCache::cacheMacros(::PPCache::Builder builder,
  ...
  for (const IncludeFileInfo& ifi : m_pp->getIncludeFileInfo()) {
    sourceMacros.insert(sourceMacros.end(), ifi.m_macroDefinitions.cbegin(),
...
void PPCache::restoreMacros(SymbolTable& targetSymbols,
  ...
  for (const ::Macro::Reader& sourceMacro : sourceMacros) {
    ...
    macros.emplace_back(m_pp->defineMacro(
...
bool PPCache::restore(PathId cacheFileId, bool errorsOnly,
  ...
  do {
    ...
    std::vector<MacroInfo*> macros;
    restoreMacros(*targetSymbols, root.getMacros(), sourceSymbols, macros);
