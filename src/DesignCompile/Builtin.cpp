...
void Builtin::addBuiltinTypes() const {
  struct FunctionDefinition {
    constexpr FunctionDefinition(std::string_view packageName, std::string_view className,
                                 std::string_view returnTypeName, std::string_view functionName,
                                 std::string_view unused1 = "", std::string_view unused2 = "")
        : packageName(packageName), className(className), returnTypeName(returnTypeName), functionName(functionName) {}
  ...
  static constexpr FunctionDefinition functionDef[] = {{"builtin", "array", "generic", "find"},
  ...
  for (const auto& f : functionDef) {
    ...
    if (package == nullptr) {
      package = new Package(m_session, f.packageName, nullptr, nullptr, InvalidNodeId, s);
    ...
    if (classDef == nullptr) {
      classDef = new ClassDefinition(m_session, f.className, nullptr, package, nullptr, InvalidNodeId, nullptr, s);
      m_design->addClassDefinition(fullClassName, classDef);
      package->addClassDefinition(f.className, classDef);
    }

    DataType* dtype = new DataType(nullptr, InvalidNodeId, f.returnTypeName, convert(f.returnTypeName));
    FunctionMethod* method = new FunctionMethod(classDef, nullptr, InvalidNodeId, f.functionName, dtype, false, false,
...
void Builtin::addBuiltinClasses() const {
  ...
  std::vector<NodeId> classes = fC1->sl_collect_all(fC1->getRootNode(), VObjectType::paClass_declaration);
  fC1->addPackageDefinition("builtin", builtinPackage);
  m_compileDesign->getCompiler()->getDesign()->addFileContent(fC1->getFileId(), fC1);

  for (const auto& classId : classes) {
    if (NodeId stId = fC1->sl_collect(classId, VObjectType::STRING_CONST, VObjectType::paAttr_spec)) {
      const std::string_view name = fC1->SymName(stId);
      const std::string fullClassName = StrCat("builtin::", name);
      ClassDefinition* classDef = m_design->getClassDefinition(fullClassName);
      if (classDef == nullptr) {
        classDef = new ClassDefinition(m_session, name, fC1->getLibrary(), builtinPackage, fC1, classId, nullptr, s);
        m_design->addClassDefinition(fullClassName, classDef);
