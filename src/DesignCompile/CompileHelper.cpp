...
bool CompileHelper::compileSignal(DesignComponent* comp,
  ...
  if (isNet) {
    ...
    if (dtype) {
      ...
      fC->populateCoreMembers(sig->getNameId(), sig->getNameId(), stv);
      if (sig->attributes()) {
        stv->Attributes(sig->attributes());
        for (auto a : *sig->attributes()) a->VpiParent(stv);
      }
      ...
      if (tps != nullptr) {
        ...
        rt->Actual_typespec(tps);
        logicn->Typespec(rt);
        NodeId typeId = sig->getInterfaceTypeNameId()
                            ? sig->getInterfaceTypeNameId()
                            : sig->getTypespecId();
        fC->populateCoreMembers(typeId, typeId, rt);
      }
      if (unpackedDimensions) {
...
*/
  ...
  while (Net_assignment) {
    ...
    if (Expression &&
      ...
      cassign->VpiParent(component->getUhdmModel());
  ...
  if (ttps == uhdmlogic_typespec) {
    ...
    if (ets != nullptr) {
      ...
      ncsize = Bits(
          ...
          fileSystem->toPathId(ets->VpiFile(), symbols),
          ets->VpiLineNo(), false);
    }
  } else if (ttps == uhdmpacked_array_typespec) {
...
bool CompileHelper::compileParameterDeclaration(
  ...
  if (fC->Type(nodeId) == VObjectType::paList_of_type_assignments) {
    ...
    while (Param_assignment) {
      ...
      p->VpiParent(pany);
      fC->populateCoreMembers(nodeId, Identifier, p);
...
int32_t CompileHelper::adjustOpSize(const typespec* tps, expr* cop,
  ...
  if (csize == 0) {
    expr* vexp = reduceExpr(
        ...
        fileSystem->toPathId(cop->VpiFile(), symbols), cop->VpiLineNo(), nullptr);
...
UHDM::expr* CompileHelper::expandPatternAssignment(
  ...
  uint64_t size =
      Bits(tps, invalidValue, component, compileDesign, Reduce::Yes, instance,
           fileSystem->toPathId(rhs->VpiFile(), symbolTable),
           rhs->VpiLineNo(), false);
