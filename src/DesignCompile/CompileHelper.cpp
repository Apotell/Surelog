  ...
  if (leaf_type == VObjectType::paDollar_keyword) {
    ...
    if (call == nullptr) {
      if (LetStmt* stmt = component->getLetStmt(name)) {
        if (m_session->getCommandLineParser()->getLetExprSubstitution()) {
...
UHDM::expr* CompileHelper::expandPatternAssignment(const typespec* tps,
  ...
  bool invalidValue = false;
  uint64_t size = Bits(tps, invalidValue, component, compileDesign, Reduce::Yes,
                       instance, fileSystem->toPathId(rhs->VpiFile(), symbols),
                       rhs->VpiLineNo(), false);
        ...
        if (taggedPattern) {
          ...
          for (any* op : *operands) {
            ...
            expr* vexp = reduceExpr(
                ...
                fileSystem->toPathId(cop->VpiFile(), symbols), cop->VpiLineNo(),
                nullptr);
