...
int32_t FunctorCompilePackage::operator()() const {
  if (CompilePackage* instance = new CompilePackage(m_session, m_compileDesign, m_package, m_design)) {
    instance->compile();
    delete instance;
  }
  return 0;
...
bool CompilePackage::compile() {
  ...
  if (ErrorContainer* errors2 = new ErrorContainer(m_session)) {
    Location loc(fC->getFileId(packId), fC->Line(packId), fC->Column(packId), symbols->getId(m_package->getName()));
      ...
      switch (type) {
        ...
        case VObjectType::paParameter_declaration: {
          ...
          if (fC->Type(list_of_type_assignments) == VObjectType::paType_assignment_list ||
            ...
            // Type param
            m_helper.compileParameterDeclaration(m_package, fC, list_of_type_assignments, false, nullptr, false, false);
          } else {
            m_helper.compileParameterDeclaration(m_package, fC, id, false, nullptr, false, false);
          }
          break;
        }
        case VObjectType::paLocal_parameter_declaration: {
          ...
          if (fC->Type(list_of_type_assignments) == VObjectType::paType_assignment_list ||
              fC->Type(list_of_type_assignments) == VObjectType::TYPE) {
            // Type param
            m_helper.compileParameterDeclaration(m_package, fC, list_of_type_assignments, true, nullptr, false, false);
          } else {
            m_helper.compileParameterDeclaration(m_package, fC, id, true, nullptr, false, false);
          }
          break;
        }
        ...
        case VObjectType::paProperty_declaration: {
          if (collectType != CollectType::OTHER) break;
          uhdm::PropertyDecl* decl = m_helper.compilePropertyDeclaration(m_package, fC, id, nullptr, nullptr);
          m_package->addPropertyDecl(decl);
          break;
        }
        case VObjectType::paSequence_declaration: {
          if (collectType != CollectType::OTHER) break;
          uhdm::SequenceDecl* decl = m_helper.compileSequenceDeclaration(m_package, fC, id, nullptr, nullptr);
          ...
          if (!sibling) {
            ...
            if (endLabel != moduleName) {
              Location loc(fC->getFileId(m_package->getNodeIds()[0]), fC->Line(m_package->getNodeIds()[0]),
                           fC->Column(m_package->getNodeIds()[0]), symbols->registerSymbol(moduleName));
