...
std::pair<bool, bool> Report::makeDiffCompUnitReport() {
  ...
  const PathId allLogFileId = fileSystem->getLogFile(false, symbols);
  const PathId unitLogFileId = fileSystem->getLogFile(true, symbols);
  const PathId diffFileId =
      fileSystem->getChild(outputDirId, kDiffLogFileName, symbols);
  ...
  const PathId allCompileDirId = fileSystem->getCompileDir(false, symbols);
  const PathId unitCompileDirId = fileSystem->getCompileDir(true, symbols);
