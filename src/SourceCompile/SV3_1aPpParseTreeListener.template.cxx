...
NodeId SV3_1aPpParseTreeListener::addVObject(antlr4::tree::TerminalNode *node,
  ...
  if (objectType == VObjectType::ppOne_line_comment) {
    ...
    NodeId pNodeId = nodeId;
    for (uint32_t i = 0; i < count; ++i) {
      nodeId = addVObject((antlr4::ParserRuleContext *)node, "\n",
                          VObjectType::ppCR);
      if (pNodeId) {
        VObject *const object = m_fileContent->MutableObject(pNodeId);
        object->m_sibling = nodeId;
      }
      pNodeId = nodeId;
    }
  } else {
...
void SV3_1aPpParseTreeListener::collectAsVisited(
    antlr4::ParserRuleContext *ctx) {
  std::vector<const antlr4::tree::ParseTree *> q;
  q.emplace_back(ctx);

  while (!q.empty()) {
    const antlr4::tree::ParseTree *const p = q.back();
    q.pop_back();

    if (p->getTreeType() == antlr4::tree::ParseTreeType::TERMINAL) {
      m_visitedNodes.emplace(p);
    } else {
      for (const antlr4::tree::ParseTree *c : p->children) {
        q.emplace_back(c);
      }
    }
  }
...
void SV3_1aPpParseTreeListener::appendPreprocEnd(antlr4::ParserRuleContext *ctx,
  ...
  std::string suffix;
  while (!tokens.empty() && StringUtils::isblank(tokens.back()->getText())) {
    suffix += tokens.back()->getText();
    tokens.pop_back();
  }
  for (size_t i = 0, ni = suffix.length(); i < ni; ++i) text.pop_back();
...
void SV3_1aPpParseTreeListener::enterMacroInstanceWithArgs(
  ...
  if (macroInfo != nullptr) {
    if (m_pp == m_pp->getSourceFile()) {
      ...
      openingIndex = m_pp->getSourceFile()->addIncludeFileInfo(
          ...
          /* sectionFileId */ m_pp->getRawFileId(),
  ...
  if (openingIndex >= 0) {
    ...
    if (emptyMacroBody && (nCRinArgs > 0)) {
      sourceLineColumnEnd.first -= nCRinArgs;
    }
...
void SV3_1aPpParseTreeListener::enterMacro_definition(
  ...
  if (body != nullptr) {
    ...
    std::string bodyText = body->getText();
    const size_t nCRs = std::count(bodyText.cbegin(), bodyText.cend(), '\n');

    if (!tokens.empty() && StringUtils::isblank(tokens.back()->getText())) {
      std::string suffix = tokens.back()->getText();
      tokens.pop_back();
      for (size_t i = 0, ni = suffix.length(); i < ni; ++i) bodyText.pop_back();
    }
    while (!tokens.empty() && (tokens.back()->getText() == "\n") &&
           !bodyText.empty() && (bodyText.back() == '\n')) {
      tokens.pop_back();
      bodyText.pop_back();
    }
    ...
    elc = ParseUtils::getEndLineColumn(tokens.empty() ? body->getStop()
                                                      : tokens.back());
    if (nCRs > 0) {
      elc.first = slc.first + nCRs;
      elc.second = bodyText.length() - bodyText.rfind('\n');
    }
  }
...
void SV3_1aPpParseTreeListener::exitComment(
  ...
  if (antlr4::tree::TerminalNode *const commentNode = ctx->One_line_comment()) {
    ...
    } else if (m_inProtectedRegion || m_inMacroDefinitionParsing) {
...
void SV3_1aPpParseTreeListener::exitEveryRule(antlr4::ParserRuleContext *ctx) {
  ...
  if ((ruleIndex == SV3_1aPpParser::RuleDescription) && m_inProtectedRegion &&
      (m_processingPragmaDirective == 0)) {
    return;
  }

  if (ruleIndex == SV3_1aPpParser::RulePragma_directive) {
    return;
  }
