...
ClassDefinition::~ClassDefinition() {
  for (auto& [k, p] : m_properties) delete p;
  for (auto& [k, p] : m_tasks) delete p;
  for (auto& [k, p] : m_constraints) delete p;
  for (auto& [k, p] : m_classes) delete p;
  for (auto& [k, p] : m_coverGroups) delete p;
}
